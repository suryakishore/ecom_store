...
import static com.app.ecomstore.util.EcomConstants.DRIVER_ASSIGN;
 ...
 */
public class DriverAssignOrderPresenter implements DriverAssignContract.PresenterOperations {
  String title = "";
  @Inject
  Utility utility;
  @Inject
  ...
  private DriverAssignContract.ViewOperations mView;
  ...
  Observer<Data> observer = new Observer<Data>() {
    ...
    public void onNext(Data value) {
      getDriverAssignOrderApi(ZERO, LIMIT);
    }
  ...
  @Inject
  public DriverAssignOrderPresenter() {
  }
  ...
  @Override
  public void attachView(DriverAssignContract.ViewOperations viewOperations) {
   ...
   */
  @Override
  public void getDriverAssignOrderApi(int skip, int limit) {
    if (mView != null) {
      mView.showProgress();
      mIsLoading = TRUE;
    }
    Utility.printLog("exe" + "DriverAssi skip" + skip + "limit" + limit);
            ...
            switch (value.code()) {
              case SUCCESS:
                try {
                  ...
                  EcomUtil.printLog("exe" + "data  5 Assign" + mOrderData.getData().size());
